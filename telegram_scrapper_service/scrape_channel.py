...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--channel",
        default=get_env("TELEGRAM_CHANNEL", "https://t.me/Bet_afri"),
        help="Telegram channel username or public link to scrape.",
    )
    parser.add_argument(
        "--start-id",
        type=int,
    ...
    parser.add_argument(
        "--output",
        default=get_env("TELEGRAM_OUTPUT_JSON", "telegram_rental_listings.json"),
        help="Path to the structured JSON output.",
    )
    parser.add_argument(
        "--assets-dir",
        default=get_env("TELEGRAM_OUTPUT_DIR", "telegram_exports"),
        help="Directory where post HTML and downloaded images are stored.",
    )
    parser.add_argument(
        "--session-name",
        default=get_env("TELEGRAM_SESSION_NAME", "telegram_scrapper_service"),
...
async def iter_rental_bundles(
    ...
    *,
    ...
    async for message in client.iter_messages(channel, offset_id=start_id or 0):
...
async def build_listing_item(
    ...
    *,
    ...
    source_link = build_message_link(channel_ref, first_message.id, channel)
    listing_id = f"{_channel_slug(channel_ref, channel)}_{first_message.id}"
    record_dir = (
        assets_dir
    ...
    item: dict[str, Any] = {
        ...
        "source_search_url": normalize_channel_ref(channel_ref),
        "title": parsed["title"],
        "category": "rental",
...
def build_message_link(channel_ref: str, message_id: int, channel: Any) -> str:
    username = getattr(channel, "username", None)
    if username:
        return f"https://t.me/{username}/{message_id}"

    parsed = urlparse(normalize_channel_ref(channel_ref))
    ...
    return normalize_channel_ref(channel_ref)
...
def _build_source_handle(channel_ref: str, channel: Any) -> str:
    username = getattr(channel, "username", None)
    if username:
        return f"@{username}"
...
def _channel_slug(channel_ref: str, channel: Any) -> str:
    username = getattr(channel, "username", None)
    if username:
    ...
    return slugify(str(getattr(channel, "id", "telegram_channel")))
